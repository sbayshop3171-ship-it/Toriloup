...
class ProductService
{
    ...
    public function list(PaginateRequest $request)
    {
        try {
            ...
            return Product::with('media', 'category', 'brand', 'taxes', 'tags', 'reviews')->withSum('stockItems', 'quantity')->with(['wishlist' => fn($query) => $query->where('user_id', Auth::check() ? Auth::user()->id : 0)])->withReviewRating()->where(function ($query) use ($requests) {
    ...
    public function store(ProductRequest $request): object
    {
        try {
            DB::transaction(function () use ($request) {
                ...
                $this->attachBarcodeImage($this->product, (int) $this->product->barcode_id, (string) $this->product->sku);
            });
            return $this->product;
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
        ...
        try {
            DB::transaction(function () use ($request, $product) {
                if ($request->barcode_id != $product->barcode_id || $request->sku != $product->sku) {
                    ...
                    $this->attachBarcodeImage($product, (int) $product->barcode_id, (string) $product->sku);
                } else {
                    $product->update($request->validated() + ['slug' => Str::slug($request->name)]);
                }
