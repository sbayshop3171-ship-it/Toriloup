...
class Iyzico extends PaymentAbstract
{
    ...
    public function payment($order, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            ...
            $buyer->setRegistrationAddress($order->address[0]?->address);
            ...
            $shippingAddress->setAddress($order->address[0]?->address);
            ...
            $billingAddress->setAddress($order->address[1]?->address);
    ...
    {
        return redirect()->route('payment.cancel', ['order' => $order, 'paymentGateway' => 'iyzico'])->with(
            'error',
