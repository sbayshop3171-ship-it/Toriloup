...
class ProductAllVariationResource extends JsonResource
{
    ...
    public function toArray($request): array
    {
        ...
        $discountedPrice = $isOfferActive ? $this->price - (($this->price / 100) * $discount) : $this->price;
        $displayPrice = app(CurrencyConversionService::class)->priceForRequest(
            (float) $discountedPrice,
            $request,
            $this->tenantFromRequest($request)
        );
        $oldDisplayPrice = app(CurrencyConversionService::class)->priceForRequest(
            (float) $this->price,
            $request,
            $this->tenantFromRequest($request)
        );
        $discountDisplay = app(CurrencyConversionService::class)->priceForRequest(
            (float) (($this->price / 100) * $discount),
            $request,
            $this->tenantFromRequest($request)
        );
        ...
        return [
            ...
            'discount_percentage'           => AppLibrary::convertAmountFormat($discount),
            'sku'                           => $this->sku,
