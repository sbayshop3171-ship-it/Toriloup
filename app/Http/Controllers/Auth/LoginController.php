    ...
    public function login(Request $request): JsonResponse
    {
        $validator = Validator::make(
            ...
            [
                ...
                'context'      => ['nullable', 'string', 'in:admin,customer'],
            ],
        );
        ...
        $roleId  = (int)$user->myRole;

        if ($context === 'admin' && $roleId === Role::CUSTOMER) {
            return new JsonResponse([
                'errors' => [
                    'validation' => 'Customer account cannot access admin panel login.',
                ],
            ], 403);
        }

        if ($context === 'customer' && $roleId !== Role::CUSTOMER) {
            return new JsonResponse([
                'errors' => [
