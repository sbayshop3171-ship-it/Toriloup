<template>
            ...
            <div class="relative" ref="notificationWrapper">
                <button
                    ...
                    <span
                        ...
                        class="absolute -top-1 -right-1 min-w-4 h-4 px-1 text-[10px] leading-4 text-center rounded-full text-white bg-primary">
    ...
    <div id="order-modal" v-if="orderNotificationStatus" ref="orderNotificationModal" class="modal active ff-modal">
        <div class="modal-dialog max-w-[360px] p-6 text-center relative">
            ...
            <router-link :to="{ path: adminRoute(orderNotification.url) }"
                @click="closeOrderNotificationModal('order-modal', 'modal-active')"
                v-if="orderNotification.url"
        ...
        window.setTimeout(() => {
            if (this.$store.getters.authStatus && this.setting.notification_fcm_api_key && this.setting.notification_fcm_auth_domain && this.setting.notification_fcm_project_id && this.setting.notification_fcm_storage_bucket && this.setting.notification_fcm_messaging_sender_id && this.setting.notification_fcm_app_id && this.setting.notification_fcm_measurement_id) {
                ...
                onMessage(messaging, (payload) => {
                    const notificationTitle = payload?.notification?.title || this.$t('label.notification');
                    ...
                    this.pushNotificationItem(payload);

                    if (this.isOrderNotification(payload) && this.orderNotification.permission) {
